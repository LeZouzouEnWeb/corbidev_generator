(function(){
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1) État initial : aucune option cochée
    $$('input[name="generate_option[]"]').forEach(i => i.checked = false);

    // 2) Tiles → ouvrir modals + cocher l'option liée
    ...
    // 3) Validation : au moins une option cochée (document-wide)
    if (form) form.addEventListener('submit', (e)=>{
      const any = document.querySelectorAll('input[name="generate_option[]"]:checked').length > 0;
      if (!any) { e.preventDefault(); alert('Sélectionne au moins un module avant de générer.'); }
    });

    // 4) ZIP-only
    if (zipBtn && zipOnly && form) zipBtn.addEventListener('click', ()=>{ zipOnly.value='1'; form.submit(); });

    // 5) Reset (tout décocher + vider les champs)
